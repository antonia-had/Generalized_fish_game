...
def optimize_fish_game():
    ...
    for selected_strategy in strategies:
        for j in range(nSeeds):
            ...
            runtime_filename = os.getcwd() + '/Reoptimized/' + str(SOW) + '/runtime/' + selected_strategy + '_' + str(j+1) + '.runtime'
            ...
            if result:
                ...
                f1 = open(os.getcwd() + '/Reoptimized/' + str(SOW) + '/sets/' + selected_strategy + '_' + str(j+1) + '.set', 'w')
                ...
                f2 = open(os.getcwd() + '/Reoptimized/' + str(SOW) + '/objs/' + selected_strategy + '_' + str(j+1) + '.obj', 'w')
                for solution in result:
                    line = ''
                    for i in range(len(solution.getObjectives())):
                        line = line + (str(solution.getObjectives()[i])) + ' '
                ...
                # Create/write only objective values to files in folder "objs", 1 file per seed. Purpose is so that
                # the file can be processed in MOEAFramework, where performance metrics may be evaluated across seeds.
                f3 = open(os.getcwd() + '/Reoptimized/' + str(SOW) + '/cnstrs/' + selected_strategy + '_' + str(j+1) + '.cnstr', 'w')
                for solution in result:
                    line = ''
