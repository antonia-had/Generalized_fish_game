...
diff = np.zeros(len(SOW_inputs))
for j in range(len(LHsamples[:,0])):
    for i in range(len(SOW_inputs)):
        diff[i] = np.absolute(SOW_inputs[i]-LHsamples[j,i])/SOW_inputs[i]
...
pts6 = ax3D.scatter(objs[:,unstable_sorted[0],0], objs[:,unstable_sorted[0],1], objs[:,unstable_sorted[0],3],
                    c=cmap(0.5),cmap=cmap, linewidth=0)
pts11 = ax3D.scatter(objs[:,3489,0], objs[:,3489,1], objs[:,3489,3],
